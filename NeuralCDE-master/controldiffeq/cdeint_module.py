...
class TimeAwareVectorField(torch.nn.Module):
    """Controlled vector field that passes time to the func.

    Use this for time-modulated vector fields like ModulatedSingleHiddenLayer.
    """
    def __init__(self, dX_dt, func):
        """Defines a time-aware controlled vector field.

        Arguments:
            dX_dt: As cdeint.
            func: A torch.nn.Module with forward(t, z) signature.
        """
        super(TimeAwareVectorField, self).__init__()
        if not isinstance(func, torch.nn.Module):
            raise ValueError("func must be a torch.nn.Module.")
...
def cdeint(dX_dt, z0, func, t, adjoint=True, time_aware=False, **kwargs):
    ...
    ```
    z_t = z_{t_0} + \int_{t_0}^t f(z_s)dX_s      (time_aware=False)
    z_t = z_{t_0} + \int_{t_0}^t f(t, z_s)dX_s   (time_aware=True)
    ```
    ...
    Arguments:
        ...
        z0: The initial state of the solution. It should have shape (..., hidden_channels), where '...' is some number
            of batch dimensions.
        func: Should be an instance of `torch.nn.Module`. Describes the vector field f(z) or f(t, z).
            - If time_aware=False: forward(z) signature, returns (..., hidden_channels, input_channels).
            - If time_aware=True: forward(t, z) signature, returns (..., hidden_channels, input_channels).
        ...
        time_aware: A boolean; whether func takes time as the first argument (default False for backward compatibility).
    ...
    # Validate func output shape
    t0_for_check = torch.zeros(1, dtype=z0.dtype, device=z0.device)
    if time_aware:
        vector_field_check = func(t0_for_check.squeeze(), z0)
    else:
        vector_field_check = func(z0)

    if vector_field_check.shape[:-2] != z0.shape[:-1]:
        raise ValueError("func did not return a tensor with the same number of batch dimensions as z0. func returned "
                         "shape {} (meaning {} batch dimensions)), whilst z0 has shape {} (meaning {} batch"
                         " dimensions)."
                         "".format(tuple(vector_field_check.shape), tuple(vector_field_check.shape[:-2]), tuple(z0.shape),
                                   tuple(z0.shape[:-1])))
    if vector_field_check.size(-2) != z0.shape[-1]:
    ...
    if vector_field_check.size(-1) != control_gradient.size(-1):
        raise ValueError("func did not return a tensor with the same number of input channels as dX_dt returned. "
                         ...
                         " channels)."
                         "".format(tuple(vector_field_check.shape), vector_field_check.size(-1), tuple(control_gradient.shape),
    ...
    # Choose appropriate vector field wrapper
    if time_aware:
        vector_field = TimeAwareVectorField(dX_dt=dX_dt, func=func)
    else:
