...
import common
import datasets
from models import vector_fields, NeuralCDE
...
def main(intensity,                                                               # Whether to include intensity or not
         ...
         modulated=False,                                                         # Whether to use ModulatedSingleHiddenLayer
         dry_run=False,
         **kwargs):                                                               # kwargs passed on to cdeint

    batch_size = 1024
    lr = 0.001 * (batch_size / 32)  # Reduced lr for stability with time modulation
    ...
    # Custom model creation for modulated mode
    if modulated and model_name == 'ncde':
        def new_make_model():
            # Use ModulatedSingleHiddenLayer for time-aware vector field
            func = vector_fields.ModulatedSingleHiddenLayer(
            ...
            return InitialValueNetwork(intensity, hidden_channels, model), func
    else:
    ...
    if dry_run:
        ...
        modulated_str = '_modulated' if modulated else ''
        name = 'sepsis' + intensity_str + modulated_str
    num_classes = 2
